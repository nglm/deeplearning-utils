...
def model_selection(
    ...
    from_batch_to_loss: Callable = from_batch_to_loss_supervised,
    batch_params: Optional[List[Dict[str, Any]]] = [{}],
    batch_params_common: Optional[Dict[str, Any]] = {},
    seed: int = 611,
    res_path: str = 'model_',
    device: Optional[torch.device] = None,
    ...
    from_batch_to_loss: Callable
        Compute the loss from a given batch
    batch_params : Optional[List[Dict[str, Any]]], optional
        List of parameter dictionaries for each batch function, by default [{}].
    batch_params_common : Optional[Dict[str, Any]], optional
    ...
    for i in range(len(model_params)):
        for j in range(len(optim_params)):
            for k in range(len(loss_params)):
                for l in range(len(batch_params)):
                    ...
                    batch_param = get_params(batch_params[l], batch_params_common)
                    ...
                    if os.path.isfile(f"{model_name}-model.pt"):
                        ...
                        losses_train, losses_val = train(
                            ...
                            from_batch_to_loss_kwargs = batch_param,
                            device=device,
                        )
...
def pipeline(
    ...
    common_params = {
        ...
        **optim_params_common,
        "from_batch_to_loss" : from_batch_to_loss, **batch_params_common,
    }
    ...
    best_model, i_best_model, model_fullpaths = model_selection(
        ...
        from_batch_to_loss = from_batch_to_loss,
        batch_params = batch_params, batch_params_common=batch_params_common,
